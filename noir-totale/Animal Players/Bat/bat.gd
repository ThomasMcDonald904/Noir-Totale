...
func _ready() -> void:
	...
	move_camera(CameraPos.THIRD_PERSON)
...
func apply_ejection(dir: Vector3):
	velocity = dir * 10

func move_camera(cam_position: CameraPos):
	var tween = get_tree().create_tween().set_trans(Tween.TRANS_EXPO)
	if cam_position == CameraPos.THIRD_PERSON:
		tween.tween_property($Camera3D, "position", Vector3(0.0, 1.95, 2.18), 1)
		tween.parallel().tween_property($Camera3D, "rotation_degrees", Vector3(-30.9, 0.0, 0.0), 1)
	elif cam_position == CameraPos.AERIAL:
		tween.tween_property($Camera3D, "position", Vector3(0.0, 7.1, 2.42), 3)
		tween.parallel().tween_property($Camera3D, "rotation_degrees", Vector3(-69.6, 0.0, 0.0), 3)
